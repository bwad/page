 ...
 */
abstract class xhp_x__base extends \xhp_xhp{}
...
if($element instanceof \xhp_x__frag){
...
if($child instanceof \xhp_xhp&&$child->categoryOf($selector)){
$result[]=$child;
}
}
}else {
$selector=\xhp_xhp::element2class($selector);
foreach($this->children as $child){
...
if($child instanceof \xhp_x__composable_element){
...
while(($composed=$that->render()) instanceof \xhp_x__element){
if(\xhp_xhp::$ENABLE_VALIDATION){
...
if($composed instanceof \xhp_x__composable_element){
$composed->transferContext($that->context);
}else if(\xhp_xhp::$ENABLE_VALIDATION){
...
case 2:
if(isset($this->children[$index])&&
!($this->children[$index] instanceof \xhp_xhp)){
++$index;
return true;
}
return false;
...
case 3:
return ':'.\xhp_xhp::class2element($rule);
...
foreach($this->children as $child){
if($child instanceof \xhp_xhp){
$tmp=':'.\xhp_xhp::class2element(get_class($child));
...
return \xhp_xhp::class2element($name);
}
}
}
...
\xhp_xhp::class2element(get_class($that)).'` reduced into `'.gettype($rend)."`.\n\n".
$that->source
);
}
}
...
\xhp_xhp::class2element(get_class($that))."`, expected array<`$type`>.\n\n".
$that->source
);
}
}
...
\xhp_xhp::class2element(get_class($that))."`, expected `$type`.\n\n".
$that->source
);
}
}
